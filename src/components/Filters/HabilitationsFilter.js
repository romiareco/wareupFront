...
export function HabilitationsFilter({ onHabilitationsChange }) {
  ...
  useEffect(() => {
    onHabilitationsChange(selectedServices);
  }, [selectedServices, onHabilitationsChange]);

  useEffect(() => {
    (async () => {
      try {
        const response = await serviceController.getServiceGroupById(
          ENV.SERVICE_GROUPS.HAB_GROUP_ID
        );
